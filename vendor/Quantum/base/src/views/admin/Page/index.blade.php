        ...
        $('.confirmDelete').on('click', function() {
            ...
            bootbox.confirm({
                ...
                callback: function(result) {
                    if (result) {
                        window.location = '{{url('admin/page')}}/' + id + '/delete';
                    }
                }
            });
        });
...
@section('breadcrumbs')
    {!! breadcrumbs(array('Dashboard' => '/admin/dashboard', 'Pages' => 'is_current')) !!}
@stop
        ...
        <div class="col-md-12">
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    {!! Form::open(array('method' => 'POST', 'url' => '/admin/pages/', 'class' => 'form-horizontal')) !!}
                    ...
                    <table class="table datatable-ajax table-bordered table-striped table-hover" id="users-table">
                        ...
                        @foreach($pages as $page)
                            <tr>
                                <td>{{$page->id}}</td>
                                <td>{{$page->title}}</td>
                                <td>{{ucfirst($page->area)}}</td>
                                <td>{{$page->route}}</td>
                                <td>{{ucfirst($page->status)}}</td>
                                <td>{{$page->created_at->format('d/m/y h:i:s')}}</td>
                                <td>{{$page->updated_at->diffForHumans()}}</td>
                                <td>
                                    <a href="{{url('admin/page/'.$page->id.'/edit')}}" class="btn bg-teal-400 btn-labeled" type="button"><b><i class="far fa-pencil"></i></b> Edit</a>
                                    &nbsp;<a href="#" class="btn btn-danger btn-labeled confirmDelete" type="button" id="" data-id='{{$page->id}}'><b><i class="far fa-times"></i></b> Delete</a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
                    ...
                    <div class="datatable-footer">
                        <div class="dataTables_info" id="DataTables_Table_3_info" role="status" aria-live="polite">
                            Showing {!! $pages->count() !!} out of {!! $pages->total() !!}
                        </div>
