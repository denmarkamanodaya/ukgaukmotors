...
@section('breadcrumbs')
    {!! breadcrumbs([Settings::get('members_home_page_title') => Settings::get('members_home_page'), 'Car Data' => 'is_current']) !!}
@stop
                    ...
                    <div class="alphaListWrap cs-ag-search">
                        <ul class="filter-list">
                            @foreach($carMakesAlpha as $key => $cList)
                                <li><a class="{!! activeLetter($key, $only) !!}" href="{!! url('/members/car-data/'.$key) !!}">{{$key}}</a></li>
                            @endforeach
                                <li><a class="" href="{!! url('/members/motorpedia') !!}">All</a></li>
                        </ul>
                    </div>
                </div>
                    ...
                    @if($carMakes)
                                @foreach ($carMakes as $carMake)
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 mb-5">
                                        ...
                                        @endif
                                        <span class="make_name_list"><a href="{{ url('/members/motorpedia/car-make/'.$carMake->slug) }}">{{ $carMake->name }}</a></span>
                                    </div>
                                @endforeach
