...
@section('breadcrumbs')
    {!! breadcrumbs(['Home' => url('/'), 'Car Data' => 'is_current']) !!}
@stop
                    ...
                    <div class="alphaListWrap cs-ag-search">
                        <ul class="filter-list">
                            @foreach($carMakesAlpha as $key => $cList)
                                <li><a class="{!! activeLetter($key, $only) !!}" href="{!! url('/car-data/'.$key) !!}">{{$key}}</a></li>
                            @endforeach
                            <li><a class="" href="{!! url('/car-data') !!}">All</a></li>
                        </ul>
                    </div>
                </div>
                <div class="row">
                    ...
                    @if($carMakes)
                                @foreach ($carMakes as $carMake)
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 mb-5">
                                        ...
                                        @endif
                                        <span class="make_name_list"><a href="{{ url('/car-make/'.$carMake->slug) }}">{{ $carMake->name }}</a></span>
                                    </div>
                                @endforeach
                    @endif
                </div>
